...
function ChromeIcon({ name }: { name: "trash" | "chevron" | "back" | "undo" | "redo" | "gear" }) {
  ...
  if (name === "gear") return <svg {...c} width={12} height={12}><circle cx="8" cy="8" r="2.2" /><path d="M8 1.8v1.6M8 12.6v1.6M14.2 8h-1.6M3.4 8H1.8M12.2 3.8l-1.1 1.1M4.9 11.1l-1.1 1.1M12.2 12.2l-1.1-1.1M4.9 4.9L3.8 3.8" /></svg>;
...
export default function EnvelopeFieldsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Separate from selectedId — selecting a field (click, place, drag) only
  // highlights it; the role/required/delete toolbar stays hidden until the
  // gear icon is explicitly clicked. Was previously tied to isSelected
  // directly, which meant a field's details popped open the instant you
  // placed it (createFieldAt already calls setSelectedId).
  ...
  function handleUndo() {
    ...
    setFields(prev);
    setSelectedId(null); setToolbarOpenId(null);
    setHistoryTick((v) => v + 1);
  }
  ...
  function handleDiscard() { setFields(savedFields); setSelectedId(null); setToolbarOpenId(null); show({ message: "Changes discarded.", type: "info" }); }
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 24, margin: "16px auto", padding: "0 24px 48px", alignItems: "flex-start" }}>
        <div style={{ width: 300, flexShrink: 0, position: "sticky", top: 24 }}>
          {selectedField ? (
            <FieldDetailsPanel field={selectedField} fields={fields} recipients={recipients} pages={pages} fitWidth={fitWidth} locked={locked} onUpdate={(patch) => updateField(selectedField.id, patch)} onRemove={() => removeField(selectedField.id)} onBack={() => { setSelectedId(null); setToolbarOpenId(null); }} />
          ) : (
              ...
              <div key={i} ref={(el) => { pageContainerRefs.current[i] = el; }} data-page-index={i} style={{ position: "relative", width: pageRenderWidth, transition: "width var(--transition-base, 180ms) ease", marginBottom: 24, boxShadow: "var(--shadow)", borderRadius: "var(--radius-sm)", overflow: "hidden" }}>
                ...
                <div
                  ...
                  onClick={(e) => {
                    ...
                    } else { setSelectedId(null); setToolbarOpenId(null); }
                  }}
                >
                  ...
                  {fields.filter((f) => f.page === i).map((f) => {
                    ...
                    return (
                      <div key={f.id}>
                        ...
                        <AnimatePresence>
                          {toolbarOpenId === f.id && !locked && (
                            <FieldToolbar key={`toolbar-${f.id}`} field={f} top={toolbarTop} left={toolbarLeft} recipients={recipients} locked={locked} onChangeRole={(role) => updateField(f.id, { role })} onToggleRequired={() => updateField(f.id, { required: !f.required })} onDelete={() => { removeField(f.id); setToolbarOpenId(null); }} />
                          )}
                        </AnimatePresence>
                      </div>
                    );
