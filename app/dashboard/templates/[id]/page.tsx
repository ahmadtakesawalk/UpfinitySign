...
export default function TemplateBuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!res.ok) {
        show({ message: "Couldn't load this template.", type: "error" });
      ...
      setHistoryIndex(0);
      setAiDrafted(Boolean(json.ai_drafted));
      setAiReviewedAt(json.ai_reviewed_at ?? null);
      setSelfServeEnabled(Boolean(json.self_serve_enabled));
      setSelfServeUrl(json.self_serve_url ?? null);
      setLoading(false);
    })();
