...
import { setTenantSetting, deleteTenantSetting } from "@/lib/settings";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  } else if (action === "set_retention_override") {
    // Enterprise-only, same reasoning as extend_trial: a specific,
    // deliberate exception on top of the tier default, not a way to
    // casually override every tenant's limits — that's what the
    // platform-wide tier_limits override (/admin/settings) is for.
    const tenant = await prisma.tenant.findUniqueOrThrow({ where: { id } });
    if (tenant.tier !== "enterprise") {
      return NextResponse.json({ error: "Per-tenant retention overrides are only available on the enterprise tier." }, { status: 400 });
    }
    const yearsRaw = (formData.get("retention_years") as string)?.trim();
    const years = Number(yearsRaw);
    if (!Number.isInteger(years) || years < 1 || years > 50) {
      return NextResponse.json({ error: "Enter a whole number of years between 1 and 50." }, { status: 400 });
    }
    await setTenantSetting(id, "retention_years", years);
    await logAdminAction(admin, "tenant.retention_override_set", id, { retention_years: years }, ip);
  } else if (action === "clear_retention_override") {
    await deleteTenantSetting(id, "retention_years");
    await logAdminAction(admin, "tenant.retention_override_cleared", id, undefined, ip);
  }
