...
export async function POST(req: NextRequest) {
  ...
  const docsPerEnvelopeRaw = (formData.get("docsPerEnvelope") as string)?.trim();
  ...
  const docsPerEnvelope = parseLimit(docsPerEnvelopeRaw ?? "");
  const retentionYears = Number(retentionRaw);

  if (envelopesPerMonth === null || aiMessagesPerMonth === null || docsPerEnvelope === null || !Number.isFinite(retentionYears) || retentionYears < 0) {
  ...
  await setPlatformSetting(
    `tier_limits.${tier}`,
    { envelopesPerMonth, aiMessagesPerMonth, retentionYears, docsPerEnvelope, addons },
    admin.id
  );
  await logAdminAction(admin, "tier_limits.updated", undefined, { tier, envelopesPerMonth, aiMessagesPerMonth, retentionYears, docsPerEnvelope, addons }, ip);
