                ...
                {canManage ? (
                  <form action="/api/admin/settings/tier-limits" method="POST" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                      ...
                      <div>
                        <label className="field-label">Documents / envelope</label>
                        <input name="docsPerEnvelope" defaultValue={fmt(limits.docsPerEnvelope)} style={{ width: 100 }} />
                      </div>
                    </div>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                    ...
                    {limits.retentionYears}-year retention, {fmt(limits.docsPerEnvelope)} doc{limits.docsPerEnvelope === 1 ? "" : "s"}/envelope. Add-ons: {limits.addons.length ? limits.addons.map((a) => ADDON_LABELS[a] ?? a).join(", ") : "none"}.
                  </p>
                )}
              </div>
            );
