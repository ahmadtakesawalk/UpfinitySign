    ...
    return (
      ...
      >
        {isImage ? (
          ...
          <span className="signature-cursive-preview" style={{ fontSize: Math.min(pos.height * 0.6, 20) }}>{value}</span>
        ) : (
 ...
 */
export function SignatureFieldInput({ field, value, error, onChange }: { field: Field; value: string; error?: string; onChange: (v: string) => void }) {
  const isImage = value.startsWith("data:image");
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {mode === "type" && (
        <div>
          <input
            className="signature-cursive-preview"
            style={{ border: "1px solid var(--border-strong)", borderRadius: "var(--radius-sm)", height: 64, padding: "0 14px" }}
            placeholder="Type your full name"
            value={isImage ? "" : value}
            onChange={(e) => onChange(e.target.value)}
          />
          <p style={{ fontSize: 11, color: "var(--text-muted)", margin: "4px 0 0" }}>Rendered in a signature-style font on the final document</p>
        </div>
      )}
...
export function SignatureModal({
  field, value, error, onChange, onClose,
}: { field: Field; value: string; error?: string; onChange: (v: string) => void; onClose: () => void }) {
  return (
    <div
      onClick={onClose}
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="card" style={{ maxWidth: 420, width: "100%" }}>
        <SignatureFieldInput field={field} value={value} error={error} onChange={onChange} />
