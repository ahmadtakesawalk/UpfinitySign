...
// sandbox's network allowlist doesn't include any TSA endpoint, so none of
// this had been run against a real TSA response as of when it was written.
//
// WIRED IN (see lib/signing/certificate.ts's generateCertificate): the
// call happens, deliberately as independent supplementary evidence
// (Certificate.timestampToken) rather than surgically re-embedded into the
// PDF's own CMS signature structure — that embedding is a materially
// different, riskier operation this doesn't attempt. The call is also
// fully non-blocking: any failure here (network, TSA rejection, or this
// parsing code hitting a real-world response shape it's never seen) is
// caught, logged, and certificate generation proceeds without it —
// nothing about signing or certificate delivery depends on this
// succeeding. That said, extractTimeStampToken's correctness against a
// REAL TSA response still hasn't been confirmed by anyone with actual
// network access to test it — check your logs after this runs live
// (Certificate.timestampObtainedAt null vs. populated) before treating a
// populated field as proof this is bulletproof, not just "didn't throw."
