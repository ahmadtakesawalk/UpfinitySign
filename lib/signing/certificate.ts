...
import { requestTimestamp, extractTimeStampToken } from "./timestamp";
...
export async function generateCertificate(envelopeId: string) {
  ...
  // RFC 3161 trusted timestamp over the actual signed document (not the
  // certificate PDF itself) — see the field comment on Certificate.
  // timestampToken in schema.prisma for why this is stored as independent
  // evidence rather than re-embedded into the PDF's own signature, and
  // the file header of ./timestamp.ts for why this is deliberately
  // non-blocking: a TSA failure (network, misconfiguration, or the
  // response-parsing code hitting a real-world edge case it's never seen)
  // must never prevent certificate generation, which is otherwise already
  // complete and correct at this point.
  let timestampToken: string | undefined;
  let timestampObtainedAt: Date | undefined;
  if (envelope.signedPdfStorageKey) {
    try {
      const signedDocBytes = await storage.get(envelope.signedPdfStorageKey);
      const digest = createHash("sha256").update(signedDocBytes).digest();
      const tsaResponse = await requestTimestamp(digest);
      timestampToken = extractTimeStampToken(tsaResponse).toString("base64");
      timestampObtainedAt = new Date();
    } catch (err) {
      await captureException(err, { context: "certificate_timestamp", envelopeId: envelope.id });
    }
  }

  const certificate = await prisma.certificate.upsert({
    where: { envelopeId: envelope.id },
    create: { envelopeId: envelope.id, pdfStorageKey: stored.key, webViewUrl: `/certificates/${envelope.id}`, timestampToken, timestampObtainedAt },
    update: { pdfStorageKey: stored.key, ...(timestampToken ? { timestampToken, timestampObtainedAt } : {}) },
  });
