...
// Typed-signature style catalog — matches DocuSign's own "pick a style"
// picker for the Type mode (as opposed to Draw/Upload, which are already
// style-agnostic — an image is an image). Shared between the frontend
// picker (components/signing/FieldRenderer.tsx) and the PDF burn step
// (lib/signing/pdf.ts), so both sides use the exact same id/font pairing
// and never drift apart. cssFontFamily is what the browser preview uses
// (loaded via app/globals.css @font-face); ttfFile is the same font
// embedded server-side at signing time, so the burned PDF matches what
// the signer actually saw and chose, not a generic fallback.
...
export const SIGNATURE_STYLES: SignatureStyle[] = [
  { id: "dancing-script", label: "Classic", cssFontFamily: "'Dancing Script', cursive", ttfFile: "signature-script.ttf" },
  { id: "great-vibes", label: "Elegant", cssFontFamily: "'Great Vibes', cursive", ttfFile: "GreatVibes-Regular.ttf" },
  { id: "sacramento", label: "Flowing", cssFontFamily: "'Sacramento', cursive", ttfFile: "Sacramento-Regular.ttf" },
  { id: "pacifico", label: "Bold script", cssFontFamily: "'Pacifico', cursive", ttfFile: "Pacifico-Regular.ttf" },
  { id: "caveat", label: "Handwritten", cssFontFamily: "'Caveat', cursive", ttfFile: "Caveat-Regular.ttf" },
];

export const DEFAULT_SIGNATURE_STYLE_ID = "dancing-script"; // matches the one font that existed before this catalog — anything with no style choice recorded (older submissions, older field data) falls back to exactly what it already looked like
