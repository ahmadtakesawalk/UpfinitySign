...
// Same reasoning as deletePlatformSetting above — a delete, not a re-write
// of the current default, so it keeps tracking the tier default going
// forward instead of freezing today's value in place.
export async function deleteTenantSetting(tenantId: string, key: string) {
  return prisma.tenantSetting.deleteMany({ where: { tenantId, key } });
}
